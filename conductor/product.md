...
3. **Private Overlay Repositories**: Proprietary file system paths, internal tooling aliases (like `antigravity`), and private `.gitconfig.local` credentials are not committed to `bareHub`. Instead, `conductor.sh` supports a secondary Stow pass from a completely isolated `~/git/dotfiles` repository if it exists on the host.
